...
function renderRankFull(r) {
  ...
  const knFullEl = document.getElementById('rank-kn-full');
  if (knFullEl) {
    knFullEl.textContent = `🧠 ${(r.kn||0).toLocaleString('vi-VN')} KN`;
  }
  ...
  // Tính % KN nếu có yêu cầu KN
  const knNeeded = r.kn_needed || 0;
  const knPctStr = (r.kn_pct !== undefined && r.kn_pct !== null) ? `, KN: ${r.kn_pct}%` : '';

  const progText = r.is_max

    ? '🏆 Đã đạt rank tối đa — Huyền thoại Anki!'

    : `Tiến tới ${r.next_rank?.label}: ${r.overall_pct}% (XP: ${r.xp_pct}%, Tiền: ${r.bal_pct}%${knPctStr})`;
  ...
  if (reqEl && r.next_rank) {
    ...
    if (knNeeded > 0) {
      ...
      <div style="background:var(--surface2);border-radius:8px;padding:8px;text-align:center">
        ...
        <div style="font-size:10px;color:var(--muted2);margin-bottom:2px">KN cần thêm</div>

        <div style="font-weight:800;color:var(--green)">🧠 ${knNeeded.toLocaleString('vi-VN')}</div>
