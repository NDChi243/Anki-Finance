...
//  PAYMENT MODAL — Cash / Credit Card / Split

// ════════════════════════════════════════════


let _paymentItem = null;
...
function showPaymentModal(itemId) {
  ...
  document.getElementById('sp-item-emoji').textContent = item.emoji || '📦';

  document.getElementById('sp-item-name').textContent = item.name;

  document.getElementById('sp-item-price').textContent = `💰 ${fmt(item.price)}`;

  // Reset UI

  selectPaymentMethod('cash');

  document.getElementById('sp-cash-amount').value = '';

  document.getElementById('sp-card-amount').value = '';

  document.getElementById('sp-split-preview').textContent = '';

  // Load user credit cards

  _loadUserCardsForPayment();

  // Show modal

  document.getElementById('modal-shop-payment').classList.add('open');
...
function closePaymentModal() {
  ...
  document.getElementById('modal-shop-payment').classList.remove('open');
...
document.getElementById('modal-shop-payment').addEventListener('click', function(e) {
...
async function _loadUserCardsForPayment() {

  try {

    const raw = JSON.parse(await B.getMyCreditCards());

    _userCards = Array.isArray(raw) ? raw : (raw.data || []);
  ...
  const sel = document.getElementById('sp-card-select');

  const info = document.getElementById('sp-card-info');
  ...
  if (!_userCards.length) {
    ...
    info.textContent = '💳 Bạn chưa có thẻ tín dụng nào. Hãy đăng ký thẻ trong tab Ngân hàng.';
  ...
  _userCards.forEach(card => {
    ...
    const available = (card.approved_limit || 0) - (card.used_credit || 0);

    opt.textContent = `${card.emoji || '💳'} ${card.product_label || 'Thẻ'} — Hạn mức còn: ${fmt(available)}`;

    opt.dataset.available = available;

    sel.appendChild(opt);

  });

  info.textContent = `✅ ${_userCards.length} thẻ khả dụng`;
...
function selectPaymentMethod(method) {
  ...
  document.querySelectorAll('.sp-method-btn').forEach(b => {
    ...
    if (m === method) {
      ...
      b.style.background = 'rgba(16,185,129,.1)';
  ...
  const cardSection = document.getElementById('sp-card-section');

  const splitSection = document.getElementById('sp-split-section');
  ...
  if (method === 'cash') {
    ...
    const item = _paymentItem;

    if (item) {

      const halfCash = Math.floor(item.price / 2 / 10000) * 10000;

      const halfCard = item.price - halfCash;

      document.getElementById('sp-cash-amount').value = halfCash;

      document.getElementById('sp-card-amount').value = halfCard;
...
function updateSplitPreview() {
  ...
  const cashAmt = parseInt(document.getElementById('sp-cash-amount').value) || 0;

  const cardAmt = parseInt(document.getElementById('sp-card-amount').value) || 0;

  const total = cashAmt + cardAmt;

  const diff = item.price - total;

  const preview = document.getElementById('sp-split-preview');

  if (diff === 0) {

    preview.innerHTML = `<span style="color:var(--green)">✅ Tổng: ${fmt(total)} — Khớp giá!</span>`;

  } else if (diff > 0) {

    preview.innerHTML = `<span style="color:var(--orange)">⚠️ Còn thiếu ${fmt(diff)}</span>`;

  } else {

    preview.innerHTML = `<span style="color:var(--red)">❌ Vượt quá ${fmt(Math.abs(diff))}</span>`;
...
async function confirmPayment() {
  ...
  const method = _paymentMethod;

  const price = item.price;

  let cashAmount = 0, cardAmount = 0, cardId = null;
  ...
  if (method === 'cash') {
    ...
    cardId = document.getElementById('sp-card-select').value;

    if (!cardId) { toast('err', '❌ Vui lòng chọn thẻ tín dụng!'); return; }
    ...
    cashAmount = parseInt(document.getElementById('sp-cash-amount').value) || 0;

    cardAmount = parseInt(document.getElementById('sp-card-amount').value) || 0;

    if (cashAmount + cardAmount !== price) {

      toast('err', `❌ Tổng thanh toán (${fmt(cashAmount+cardAmount)}) không khớp giá (${fmt(price)})`);
  ...
  try {
    ...
    const res = JSON.parse(await B.processShopPayment(item.id, paymentJson));

    if (res.ok) {

      closePaymentModal();

      let msg = `✅ Đã mua ${res.item_name}! Còn: ${fmt(res.new_balance)}`;

      if (res.card_used > 0) {

        msg += ` | 💳 Thẻ: ${fmt(res.card_used)}`;
      ...
      toast('err', '❌ ' + (res.error || 'Thanh toán thất bại'));
...
function openItemDetail(itemId) {
  ...
  // Price & buy button — luôn cho phép click để mở payment modal
  ...
  btn.textContent = can ? '🛒 Mua ngay' : '💳 Mua bằng thẻ tín dụng';

  btn.disabled = false;

  btn.className = `btn ${can?'btn-primary':'btn-credit'}`;
...
function buyItemFromDetail() {
  ...
  // Show payment modal instead of buying directly

  closeItemDetail();

  setTimeout(() => showPaymentModal(_detailItem.id), 300);
